-- ===========================================
-- Base de datos Electroservicios adaptada a ISO 9001 / IATF 16949
-- ===========================================

DROP DATABASE IF EXISTS electroservicios_db_normas;
CREATE DATABASE electroservicios_db_normas
...
-- ===========================================
-- EXTENSIONES DE CALIDAD (ISO 9001 / IATF 16949)
-- ===========================================
...
CREATE TABLE documentos_calidad (
    ...
    tipo ENUM('manual','procedimiento','instruccion','formato','otro') NOT NULL,
    version VARCHAR(20) NOT NULL,
    estado ENUM('vigente','obsoleto','borrador') DEFAULT 'vigente',
    fecha_emision DATE NOT NULL,
    responsable_id INT,
    archivo VARCHAR(255),
    observaciones TEXT,
    FOREIGN KEY (responsable_id) REFERENCES entidades(id_entidad)
        ON UPDATE CASCADE ON DELETE SET NULL
);
...
CREATE TABLE no_conformidades (
    ...
    origen ENUM('cliente','proveedor','proceso_interno','auditoria') NOT NULL,
    descripcion TEXT NOT NULL,
    id_responsable INT,
    estado ENUM('abierta','en_proceso','cerrada') DEFAULT 'abierta',
    fecha_cierre DATE,
    FOREIGN KEY (id_responsable) REFERENCES entidades(id_entidad)
        ON UPDATE CASCADE ON DELETE SET NULL
);
...
CREATE TABLE acciones_correctivas (
    ...
    fecha_inicio DATE,
    fecha_fin DATE,
    estado ENUM('pendiente','en_proceso','implementada','verificada') DEFAULT 'pendiente',
...
CREATE TABLE auditorias (
    ...
    tipo ENUM('interna','externa','proveedor') NOT NULL,
    fecha DATE NOT NULL,
    auditor VARCHAR(150),
    norma_aplicable ENUM('ISO9001','IATF16949','otro'),
    estado ENUM('planificada','realizada','cerrada') DEFAULT 'planificada',
    informe TEXT
);
...
CREATE TABLE hallazgos_auditoria (
    ...
    id_auditoria INT,
    descripcion TEXT NOT NULL,
    clasificacion ENUM('observacion','no_conformidad_menor','no_conformidad_mayor') NOT NULL,
    responsable_id INT,
...
CREATE TABLE riesgos_oportunidades (
    ...
    tipo ENUM('riesgo','oportunidad') NOT NULL,
    descripcion TEXT NOT NULL,
